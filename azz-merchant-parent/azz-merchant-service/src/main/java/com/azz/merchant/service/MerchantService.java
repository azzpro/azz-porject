        ...
        for (int i = 1; i <= lastRowNum; i++) {
        	...
        	if (ObjectUtils.isNotNull(row)) {
        		...
        		MerchantUser userRecord = MerchantUser.builder().createTime(nowDate).creator(creator)
        			...
        			.postName(postName).merchantUserCode(SystemSeqUtils.getSeq(code))
        			.merchantUserName(merchantUserName).merchantCode(merchantCode).salt(pwd.getSalt()).build();
        		merchantUserMapper.insertSelective(userRecord);
        		// 用户与角色绑定
        		MerchantUserRole userRoleRecord = MerchantUserRole.builder()
        				.createTime(nowDate)
        				.creator(creator) 
        				.merchantUserId(userRecord.getId())
        				.roleId(roleId).build();
        		merchantUserRoleMapper.insertSelective(userRoleRecord);
        	}
        }
